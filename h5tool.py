...
def create_celeba_hq(h5_filename, celeba_dir, delta_dir, num_threads=4, num_tasks=100):
    ...
    if len(glob.glob(glob_pattern)) != glob_expected:
        ...
        return
    with open(os.path.join(delta_dir, 'image_list.txt'), 'rt') as file:     # open for reading txt
    ...
    def process_func(idx):
        # Load original image.
        orig_idx = fields['orig_idx'][idx]    # 000001
        orig_file = fields['orig_file'][idx]   # 000002.jpg
        orig_path = os.path.join(celeba_dir, 'img_celeba', orig_file)   # celeba_dir\img_celeba\000001.jpg
        ...
        kdf = cryptography.hazmat.primitives.kdf.pbkdf2.PBKDF2HMAC(algorithm=algorithm, length=32, salt=orig_file.encode('utf-8'), iterations=100000, backend=backend)
